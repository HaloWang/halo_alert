...
class Alert {
  static Widget deploy() {
    return UncontrolledProviderScope(
      ...
      child: const IgnorePointer(child: _Alert()),
    );
  }
      ...
      child: Stack(
        children: items.indexMap((index, value) {
          ...
          final double? top = switch (item.position) {
            AlertPosition.top => item.displayStatus == AlertDisplayStatus.show
                ? paddingTop + 12
                : paddingTop + -50,
            AlertPosition.bottom => null,
            AlertPosition.center =>
              item.displayStatus == AlertDisplayStatus.show ? -10 : 10,
          };

          final double? bottom = switch (item.position) {
            AlertPosition.top => null,
            AlertPosition.bottom =>
              item.displayStatus == AlertDisplayStatus.show
                  ? paddingBottom + 34
                  : paddingBottom + -50,
          ...
          final duration =
              item.displayStatus == AlertDisplayStatus.show ? 250.ms : 150.ms;

          return AP(
            key: key,
            duration: duration,
            curve: item.displayStatus == AlertDisplayStatus.show
                ? Curves.easeOutBack
                : Curves.easeInBack,
            top: top,
            bottom: bottom,
